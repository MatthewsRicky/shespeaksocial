...
const ServiceCards = () => {
	...
	return (
		<div className="flex items-center justify-center h-fit bg-linear-to-br flex-col py-10">
			<header>
				<h1 className="font-bold text-3xl">How can I help you</h1>
				</header>
				<div className="flex flex-col md:flex-row items-center justify-between px-4 py-3 w-fit lg:w-[80%] mx-auto rounded-lg text-slate-900">

					{service.map((service) => (
						<section  key={service.id} className="flex  md:w-[95%] items-center justify-center mx-auto odd:translate-y-[-10] even:translate-y-10">
							<div className="flex flex-col items-center justify-around text-center h-[40%] w-[90%] md:w-[80%] rounded-lg  min-h-[28rem] py-4 my-2 sm:m-3 lg:m-4 bg-linear-to-b from-violet-50 to-pink-100 shadow-violet-600/20 shadow-xl hover:scale-105 transition-all ease-in-out">
								<Image src={service.imgUrl} alt="make-money icon" width={50} height={50} className="h-[50%] w-fit"/>
