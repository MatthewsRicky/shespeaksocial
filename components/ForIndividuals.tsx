		...
		<div className="flex justify-center items-center shadow-lg rounded-lg shadow-violet-300 gap-8 m-4 px-3 py-4">
			{individual.map((item) => (
				<div key={item.id} className="flex flex-col items-center justify-center p-2 shadow-md shadow-pink-200 gap-y-4">
					...
					<button className="border px-3 py-2 rounded-sm bg-pink-400/90 text-slate-800 hover:scale-105 hover:bg-pink-500 hover:text-slate-200 transition-all duration-300 hover:shadow-pink-500 hover:shadow-sm hover:border-pink-500 border-pink-300">{item.btnlabel}</button>
				</div>
			))}
		</div>
	)
